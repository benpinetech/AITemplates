...
auto-persist flow to keep one rewrite per (agency, scope, JDA) when the
...
def _accepted_files(tmp_path, agency, scope):
    agency_dir = tmp_path / "verified" / agency
    if scope[0] == ss.SCOPE_TEMPLATE:
        d = agency_dir / "by_template" / scope[1]
    elif scope[0] == ss.SCOPE_AUDIENCE:
        d = agency_dir / "by_audience" / scope[1]
    else:
        d = agency_dir / "global"
...
class TestPruneConflictingInScope:
    def test_removes_prior_with_same_jda_different_pine(self, tmp_path):
        ...
        ss.accept_suggestion(JDA, PINE_OLD, agency="oba", scope=scope, root=tmp_path)
        assert len(_accepted_files(tmp_path, "oba", scope)) == 1

        removed = ss.prune_conflicting_in_scope(
            JDA, PINE_NEW, agency="oba", scope=scope, root=tmp_path,
        )
    ...
    def test_idempotent_when_existing_match_is_identical(self, tmp_path):
        ...
        ss.accept_suggestion(JDA, PINE_OLD, agency="oba", scope=scope, root=tmp_path)
        removed = ss.prune_conflicting_in_scope(
            JDA, PINE_OLD, agency="oba", scope=scope, root=tmp_path,
        )
        assert removed == []
        assert len(_accepted_files(tmp_path, "oba", scope)) == 1
    ...
    def test_does_not_touch_other_scopes(self, tmp_path):
        ...
        ss.accept_suggestion(JDA, PINE_OLD, agency="oba", root=tmp_path)  # global
        scope = (ss.SCOPE_TEMPLATE, "Letter to C.rtf")
        removed = ss.prune_conflicting_in_scope(
            JDA, PINE_NEW, agency="oba", scope=scope, root=tmp_path,
        )
    ...
    def test_does_not_touch_other_jda_matches(self, tmp_path):
        ...
        ss.accept_suggestion(
            ...
            agency="oba", scope=scope, root=tmp_path,
        )
        removed = ss.prune_conflicting_in_scope(
            JDA, PINE_NEW, agency="oba", scope=scope, root=tmp_path,
        )
    ...
    def test_accept_after_prune_replaces_cleanly(self, tmp_path):
        scope = (ss.SCOPE_TEMPLATE, "Letter to C.rtf")
        ss.accept_suggestion(JDA, PINE_OLD, agency="oba", scope=scope, root=tmp_path)
        ss.prune_conflicting_in_scope(JDA, PINE_NEW, agency="oba", scope=scope, root=tmp_path)
        ss.accept_suggestion(JDA, PINE_NEW, agency="oba", scope=scope, root=tmp_path)
