<script>
  // Settings dialog — OpenAI credentials (API key + model). Persisted
  // via window.api.{getSettings, setSettings}. The main process merges
  // both into the env it passes to the Python pipeline.
  ...
  // Snapshot of the last-loaded values so Cancel can revert cleanly
  // and Save knows what to mask in the API-key placeholder.
    ...
    >
      ...
      <div class="body">
        <label class="field">
          ...
          <small class="help">
            Stored locally in your user data folder. Falls back to
            <code>OPENAI_API_KEY</code> in the repo-root <code>.env</code> if blank.
          </small>
        </label>
        ...
        <label class="field">
          ...
          <input
            ...
            placeholder="gpt-5.5 (default)"
            spellcheck="false"
          />
          <small class="help">
            Sets <code>OPENAI_MODEL</code> for the LLM fallback. Leave blank for the
            pipeline default.
          </small>
        </label>
