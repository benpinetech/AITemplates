...
function runPipeline(rtfPath, org) {
  ...
  const args = usePackagedSidecar
    ? [rtfPath, "--org", org, "--json"]
    : ["-m", "Agent.v2.tools.convert", rtfPath, "--org", org, "--json", "--quiet"];
...
function runPersistEdit(payload) {
  ...
  const cmd = usePackagedSidecar
    ? path.join(process.resourcesPath, "binaries", SIDECAR_NAME)
    : VENV_PYTHON;
  // The packaged sidecar will need its own --mode flag eventually;
  // for now, dev-mode is the only supported persist path because the
  // PyInstaller bundle just wraps convert.py.
  if (usePackagedSidecar) {
    return Promise.resolve({
      error: "persistEdit is not yet wired in packaged builds",
    });
  }
  const args = [
    "-m", "Agent.v2.tools.persist_suggestion",
  ...
  return new Promise((resolve) => {
    ...
    const child = spawn(cmd, args, { cwd: REPO_ROOT, env: pipelineEnv() });
    let stdout = "";
    let stderr = "";
    child.stdout.on("data", (d) => { stdout += d.toString("utf-8"); });
