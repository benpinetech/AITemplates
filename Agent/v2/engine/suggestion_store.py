...
def _resolve_root(root: Optional[Path]) -> Path:
    """Pick the on-disk root: caller-supplied wins, then env var, then
    the project default. Used by every read/write entry point below so
    ``JDAPINE_SUGGESTIONS_ROOT`` Just Works."""
    import os
    if root is not None:
        return Path(root)
    env_root = os.environ.get("JDAPINE_SUGGESTIONS_ROOT")
    if env_root:
